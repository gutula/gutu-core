...
- first-party business runtime primitives for numbering, localization, tax, import quarantine, traceability, reconciliation, contract registry checks, pack preview or rollback, SQL-backed business state stores, durable outbox or inbox or dead-letter or projection orchestration, shared Postgres or SQLite schema builders, and install-guidance-aware dependency evaluation
- business-suite scaffolding for 25 first-party business plugin repos, 13 business pack artifacts, repo-local CI, ERP parity catalogs, 11 named cross-plugin end-to-end business flows with durable report artifacts, full-suite resilience verification across all 25 business plugins, local signing or promotion flows, direct cross-plugin contract scenario verification, and generated install guidance that separates required versus recommended composition
